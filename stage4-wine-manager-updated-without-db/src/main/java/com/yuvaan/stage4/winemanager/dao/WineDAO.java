package com.yuvaan.stage4.winemanager.dao;
...
public class WineDAO {
	...
	private List<Wine> wineList = new ArrayList<>(Arrays.asList(new Wine(0, "CHATEAU DE SAINT COSME", 2009, 13.0f,
			...
			"Batch-AAA-83763637"),
			new Wine(1, "LAN RIOJA CRIANZA", 2006, 10.0f, "Spain", "Rioja",
					...
					"Batch-ABF-82373"),
			new Wine(2, "MARGERUM SYBARITE", 2010, 13.0f, "USA", "California Central Cosat",
					...
					"Batch-AOA-83737"),
			new Wine(3, "OWEN ROE \"EX UMBRIS\"", 2009, 12.0f, "USA", "Washington",
					...
					"Batch-AAA-2897363"),
			new Wine(4, "REX HILL", 2009, 14.0f, "USA", "Oregon",
					...
					"Batch-AAA-7235636"),
			new Wine(5, "VITICCIO CLASSICO RISERVA", 2007, 13.0f, "Italy", "Tuscany",
					...
					"Batch-XAA-937372"),
			new Wine(6, "CHATEAU LE DOYENNE", 2005, 11.0f, "France", "Bordeaux",
					...
					"Batch-AXA-623932"),
			new Wine(7, "DOMAINE DU BOUSCAT", 2009, 10.0f, "France", "Bordeaux",
					...
					"Batch-AAB-837647"),
			new Wine(8, "BLOCK NINE", 2009, 13.5f, "USA", "California",
					...
					"Batch-AAA"),
			new Wine(9, "DOMAINE SERENE", 2007, 13.0f, "USA", "Oregon",
					...
					"Batch-AAA-36357"),
			new Wine(10, "BODEGA LURTON", 2011, 12.0f, "Argentina", "Mendoza",
					"Solid notes of black currant blended with a light citrus make this wine an easy pour for varied palates.",
					"Batch-AAA283773"),
			new Wine(11, "LES MORIZOTTES", 2009, 11.0f, "France", "Burgundy",
	...
	public List<Wine> findWineByName(final String wineName) {
		...
		if (!resultWineList.isEmpty())
			return resultWineList;
		else
			throw new WineNotFoundException("No Wine list exists.");

	}

	public Wine findWineById(final int wineId) {
		Wine resultWine = findWine(wineId);
		if (resultWine != null)
			return resultWine;
		else
			throw new WineNotFoundException("No Wine exists with Id: " + wineId);
	}

	public void deleteWineById(final int wineId) {
		Wine resultWine = findWine(wineId);
		if (resultWine != null)
			wineList.remove(resultWine);
		else
			throw new WineNotFoundException("No Wine exists with Id: " + wineId);
	}

	public boolean updateWine(Wine updatedWine) {
		boolean isUpdated = false;
		int updateWineId = updatedWine.getId();
		Wine existingWine = findWine(updateWineId);
		if (existingWine != null) {
			int index = wineList.indexOf(existingWine);
			wineList.remove(index);
			wineList.add(index, updatedWine);
			isUpdated = true;
		} else {
	...
	private Wine findWine(final int id) {
